function load_neighbour_graph(data, graphid){
    ...
    $.ajax({
        url: "./data/"+data.tissue.toUpperCase()+"/"+data.condition+"_full.csv",
        type: 'get',
        dataType: 'csv',
        success: function(data) {
            data_attr = $.csv.toObjects(data.query.results.body.p);
        },
        error: function(jqXHR, _, _){
            data_attr = $.csv.toObjects(jqXHR['responseText']);
        },
        complete: function (jqXHR, status) {
            var nodes = data.nodes;
            for (var i = 0; i < nodes.length; i += 1){
                let attr = data_attr.find(d => d.Label === nodes[i].label);
                if(attr.Symbol !== "")
                    nodes[i].label = attr.Symbol.split(",")[0];
            }
            ...
            // $(document).ready(()=>{
                ...
                $('#originalN').click(()=>{
                    ...
                    $("#color-textN").html("Node colour grading is proportional to its degree");
                    $("#pathN").val('default');
                    $("#pathHeadN > div > span").html('- None -');
                });
                $('#moduleN').click(()=>{
                    ...
                    $("#color-textN").html("Node colour indicates module membership");
                    $("#pathN").val('default');
                    $("#pathHeadN > div > span").html('- None -');
                });
